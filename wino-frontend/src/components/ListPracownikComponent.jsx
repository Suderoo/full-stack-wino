...
import { deletePracownik, listPracownicy } from '../services/PracownikService';
...
const ListPracownikComponent = () => {
    ...
    function removePracownik(id){
        ...
        deletePracownik(id).then((response) =>{
            getAllPracownicy();
        }).catch(error =>{
            console.error(error);
        })
                    ...
                    {pracownicy.map(pracownik => 
                        <tr key={pracownik.id}>
                            ...
                            <td>
                                ...
                                <button className='btn btn-danger' onClick={() => removePracownik(pracownik.id)} style={{marginLeft: '10px'}}>Usuń</button>
                            </td>
                        </tr>
                    )}
                </tbody>
